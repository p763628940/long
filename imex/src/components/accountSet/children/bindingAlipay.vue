	...
	<div>
		...
		<ul class="userList">
			<li>
				...
				<input type="text" v-model="userInfo.trueName" disabled/>
			</li>
			<li>
				<span class="name">{{$t("userCenter.Alipayaccount")}}</span>
				<input type="text" v-model="userList.WeChatNum" :placeholder="$t('userCenter.EnAlipayaccount')" />
			</li>
			<li>
				<span class="name">{{$t("userCenter.Fundpassword")}}</span>
				<input type="password" v-model="userList.CapitalCipher" :placeholder="$t('userCenter.EnterFundpassword')" />
			</li>
		</ul>
		<!--上传二维码-->
		<div class="upload">
				...
				<div class="Uploading_files">
					<div class="Uploading_files_left">
						<p v-show="SelectWeChatImg">
							...
							<span>{{$t("userCenter.ChooseAlipaycode")}}.jpg.png.jpeg</span>
						</p>
						<p v-show="!SelectWeChatImg">
...
<script>
	...
	export default {
		data() {
			return {
				headerTile: this.$t("userCenter.BindAlipay"),
				userList: {
					WeChatNum: "",
					/*微信账号*/
					CapitalCipher: "",
